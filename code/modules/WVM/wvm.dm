...
#define VENDORLINE_IDLE 1
#define VENDORLINE_WELCOME 2
#define VENDORLINE_CANT_AFFORD 3
#define VENDORLINE_SELL_THING 4
#define VENDORLINE_BAD_FACTION 5


/**********************Trading Protectron Vendors**************************/

/obj/structure/wasteland_vendor
	name = "A cool vendor"
	desc = "Hi! I'm a vendor! Buy stuff from me using your bank account money."
	icon = 'icons/WVM/machines.dmi'
	icon_state = "weapon_idle"
	...
	var/list/messages_idle = list(
		"Hey, come buy %A_RANDOM_STOCK!"
	)
	var/list/messages_welcome = list(
		"Welcome to my shop, %PLAYERNAME! You would probably find %A_RANDOM_STOCK handy."
	)
	var/list/messages_sale = list(
		"Here you go, %PLAYERNAME!"
	)
	var/list/messages_cantafford = list(
		"I'm sorry, %PLAYERNAME! I can't give credit!"
	)
	var/list/messages_badfaction = list(
		"Get out, %PLAYERNAME! Youre bad news!"
	)
	var/next_idle_speak = 0
	var/next_idle_low = 3 MINUTES
	var/next_idle_high = 10 MINUTES
	var/mob/living/simple_animal/mouse/vendormouse/speaker
	var/chatcolor = "#567DEF"
	var/list/factionlock = list()

/obj/structure/wasteland_vendor/Initialize()
	. = ..()
	START_PROCESSING(SSprocessing, src)
	speaker = new(src)
	speaker.name = name
	speaker.chat_color = chatcolor
	speaker.chat_color_darkened = chatcolor
	speaker.chat_color_name = speaker.name

/obj/structure/wasteland_vendor/Destroy()
	STOP_PROCESSING(SSprocessing, src)
	QDEL_NULL(speaker)
	. = ..()

/obj/structure/wasteland_vendor/process()
	if(world.time < next_idle_speak)
		return
	speak(VENDORLINE_IDLE)
	next_idle_speak += rand(next_idle_low, next_idle_high)
...
/obj/structure/wasteland_vendor/proc/speak(kind, mob/user, datum/data/wasteland_equipment/thing)
	switch(kind)
		if(VENDORLINE_IDLE)
			. = safepick(messages_idle)
		if(VENDORLINE_WELCOME)
			. = safepick(messages_welcome)
		if(VENDORLINE_CANT_AFFORD)
			. = safepick(messages_cantafford)
		if(VENDORLINE_SELL_THING)
			. = safepick(messages_sale)
	...
	if(rand)
		randomware = "\a [rand.equipment_name]"
	. = replacetext(., "%PLAYERNAME", replaceplayername)
	. = replacetext(., "%A_RANDOM_STOCK", randomware)
	if(speaker)
		speaker.say(.)
	else
		say(.)

/obj/structure/wasteland_vendor/proc/can_faction(mob/living/user)
	. = TRUE
	if(!LAZYLEN(factionlock))
		return TRUE
	if(!isliving(user))
		return TRUE
...
/obj/structure/wasteland_vendor/ui_interact(mob/user)
	...
	dat +="<div class='statusDisplay'>"
	dat += "<b>Hi [user]!</b><br>"
	dat += "<b>You have</b> [maptext_pre]!<br>"
	dat += "</div>"
	dat += "<br>"
	// dat +="<div class='statusDisplay'>"
	...
	dat += "<br>"
	dat +="<div class='statusDisplay'>"
	dat += "<b>Heres what I have for sale:</b><BR><table border='0' width='300'>"
	var/cha_mod = SSmobs.stat_roll_vendor_multiplier(user)
	...
	for(var/datum/data/wasteland_equipment/prize in prize_list)
		var/baseprice = prize.cost
		var/adj_price = baseprice * cha_mod
		var/difference = adj_price - baseprice
		var/modtext = ""
		if(difference > 0)
			modtext = span_alert(" +[difference] (CHA)")
		if(difference < 0)
			modtext = span_green(" -[difference] (CHA)")
		dat += "<tr><td>[prize.equipment_name]</td><td>[baseprice][modtext]</td><td><A href='?src=[REF(src)];purchase=[REF(prize)]'>Purchase</A></td></tr>"
	dat += "</table>"
	dat += "</div>"
...
/obj/structure/wasteland_vendor/Topic(href, href_list)
	if(..())
		return
	...
	if(href_list["purchase"])
		...
		if(!QB)
			return
		var/cashhold = CREDITS_TO_COINS(QB.unclaimed_points)
		// var/basecost = prize.cost
		var/actual_cost = prize.cost
		var/cha_mod = SSmobs.stat_roll_vendor_multiplier(user)
		actual_cost *= cha_mod
		// var/difference = basecost - actual_cost
		if(actual_cost > cashhold)
			speak(VENDORLINE_CANT_AFFORD, user, prize)
			updateUsrDialog()
			return
		QB.adjust_funds(-COINS_TO_CREDITS(actual_cost))
		GLOB.vendor_cash += actual_cost
		speak(VENDORLINE_SELL_THING, user, prize)
...
/obj/structure/wasteland_vendor/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/f13Cash))
		var/datum/quest_book/QB = SSeconomy.get_quest_book(user)
		if(!QB)
			to_chat(user, span_alert("Oh no you dont have a quest book!"))
			return
...
/* Adding a caps to caps storage and release vending item. */
/obj/structure/wasteland_vendor/proc/add_caps(obj/item/I)
...
/* Spawn all caps on world and clear caps storage */
/obj/structure/wasteland_vendor/proc/remove_all_caps()
	if(stored_caps <= 0)
		return
...
/mob/living/simple_animal/mouse/vendormouse
	name = "radmouse"
	desc = "It's the rat that sells all of the wares!"
