	...
	switch(current_tab)
		if(SETTINGS_TAB) // Character Settings#
			...
			if(jobban_isbanned(user, "appearance"))
				...
				dat += "<b>congrats on being the first one to be identity banned in the history of furry ERP servers. wow</b><br>"
			dat += "<b>Name:</b> <a href='?_src_=prefs;preference=name;task=input'>[real_name]</a><BR>"
			var/mygender = "Unspecified"
			switch (gender)
				if (MALE)
					mygender = "Male"
				if (FEMALE)
		...
		if(CHAR_INFO_TAB)
			dat += "<center>"
			dat += "u shouldnt see this, call Superlagg if u can see this<br>"
			dat += "</center>"
