...
/datum/controller/subsystem/ticker/proc/catgirl_report(popcount)
	...
	for(var/atom/thingy in important_things)
		if (thingy == null)
			continue // We have no idea what the thing is
		var/turf/where = get_turf(thingy)
		if (where == null)
			continue // We have no idea where the thing is
		var/murine_got_it = where.z != onmap_z || istype(get_area(where), /area/f13/ambientlighting/building/murrines_score_here) // went off the map
